...
export default function IndustrySelection() {
  ...
  useEffect(() => {
    ...
    // Background transition triggered by the specific line in TextReveal section
    const textRevealSection = document.querySelector('#text-reveal');
    const transitionTriggerLine = textRevealSection?.querySelector('[data-testid="text-phrase-5"]'); // "six years in consulting..." line
    
    if (transitionTriggerLine) {
      ScrollTrigger.create({
        trigger: transitionTriggerLine,
        start: "top 70%",
        end: "bottom 50%",
        scrub: 1,
        onUpdate: (self) => {
          const progress = self.progress;
          // Transition this section's background to white
          if (progress > 0) {
            gsap.to(section, {
              background: `linear-gradient(to bottom, 
                rgba(255, 122, 51, ${1 - progress}) 0%, 
                rgba(255, 138, 74, ${1 - progress * 0.8}) 8%, 
                rgba(255, 179, 128, ${1 - progress * 0.6}) 25%, 
                rgba(255, 217, 204, ${1 - progress * 0.4}) 45%, 
                rgba(255, 242, 238, ${1 - progress * 0.2}) 65%, 
                #FFFFFF 80%, 
                #FFFFFF 100%)`,
              duration: 0.1
            });
    ...
    return () => {
      ScrollTrigger.getAll().forEach(trigger => {
        if (trigger.trigger === section || trigger.trigger === transitionTriggerLine) {
          trigger.kill();
        }
      });
    };
  ...
  return (
    <section 
      ...
      data-testid="industry-selection"
    >
